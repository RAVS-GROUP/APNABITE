...
(function(window, document) {
  ...
  function collectElements() {
    elements.refreshButton =
      byId('customer-cart-refresh-button');

    elements.loading =
      byId('customer-cart-loading');

    elements.error =
      byId('customer-cart-error');

    elements.errorTitle =
      byId('customer-cart-error-title');

    elements.errorMessage =
      byId('customer-cart-error-message');

    elements.retryButton =
      byId('customer-cart-error-retry');

    elements.empty =
      byId('customer-cart-empty');

    elements.content =
      byId('customer-cart-content');

    elements.kitchenName =
      byId('customer-cart-kitchen-name');

    elements.addMore =
      byId('customer-cart-add-more');

    elements.itemSummary =
      byId('customer-cart-item-summary');

    elements.clearButton =
      byId('customer-cart-clear-button');

    elements.items =
      byId('customer-cart-items');

    elements.changeAddress =
      byId('customer-cart-change-address');

    elements.addressCard =
      byId('customer-cart-address-card');

    elements.addressLabel =
      byId('customer-cart-address-label');

    elements.addressDistance =
      byId('customer-cart-address-distance');

    elements.addressLine =
      byId('customer-cart-address-line');

    elements.addressReceiver =
      byId('customer-cart-address-receiver');

    elements.addressWarning =
      byId('customer-cart-address-warning');

    elements.offerButton =
      byId('customer-cart-offer-button');

    elements.offerTitle =
      byId('customer-cart-offer-title');

    elements.offerMessage =
      byId('customer-cart-offer-message');

    elements.paymentMethods =
      byId('customer-cart-payment-methods');

    elements.paymentOnline =
      byId('customer-cart-payment-online');

    elements.paymentCod =
      byId('customer-cart-payment-cod');

    elements.paymentCodOption =
      byId('customer-cart-payment-cod-option');

    elements.paymentCodMessage =
      byId('customer-cart-payment-cod-message');

    elements.itemTotal =
      byId('customer-cart-item-total');

    elements.deliveryFee =
      byId('customer-cart-delivery-fee');

    elements.platformFee =
      byId('customer-cart-platform-fee');

    elements.discountRow =
      byId('customer-cart-discount-row');

    elements.discount =
      byId('customer-cart-discount');

    elements.rewardRow =
      byId('customer-cart-reward-row');

    elements.rewardDiscount =
      byId('customer-cart-reward-discount');

    elements.finalTotal =
      byId('customer-cart-final-total');

    elements.minimumWarning =
      byId('customer-cart-minimum-warning');

    elements.freeDeliveryMessage =
      byId('customer-cart-free-delivery-message');

    elements.instructionInput =
      byId('customer-cart-instruction-input');

    elements.checkoutBar =
      byId('customer-cart-checkout-bar');

    elements.footerTotal =
      byId('customer-cart-footer-total');

    elements.continueButton =
      byId('customer-cart-continue-button');
  }
  ...
  function normalizeCart(cart) {
    ...
    return {
      ...
      cartId:
        cleanText(input.cartId),

      kitchenId:
        cleanText(input.kitchenId),

      kitchenName:
        cleanText(input.kitchenName),
  ...
  function loadCart() {
    try {
      if (
        window.ApnaBiteCartSync &&
        typeof window.ApnaBiteCartSync
          .getCart === 'function'
      ) {
        state.cart =
          normalizeCart(
  ...
  function loadStoredAddressPreview() {
    for (
      let index = 0;
      index <
      SELECTED_ADDRESS_KEYS.length;
  ...
  async function clearCart() {
    ...
    if (
      ...
      try {
        await window.ApnaBiteCartSync
          .clear();
      } catch (error) {
        showToast(
          'Cart cleared on this device. Server update could not be completed.',
          'warning'
        );
  ...
  async function syncCartBeforeOrder() {
    ...
    if (!result) {
      throw new Error(
        'Cart could not be synced. Check your connection and try again.'
      );
    }
  ...
  function setPlacingOrder(placing) {
    ...
    elements.paymentOnline.disabled =
      state.placingOrder;
  ...
  async function loadCheckout() {
    ...
    try {
      ...
      /*
       * Server cart and addresses load
